...
- Dependencies: refreshed production runtime dependencies within declared ranges and raised the declared minimums for React, React DOM, and Multer to the verified locked versions.
- Build: updated the browser TypeScript config to Vite's bundler module resolution and added local Vite/lucide declarations so local typecheck remains clean on current TypeScript.
